 ...
 */
final class Version20260222005312 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        $this->addSql('CREATE TABLE user (id INT AUTO_INCREMENT NOT NULL, nom VARCHAR(255) NOT NULL, prenom VARCHAR(255) NOT NULL, email VARCHAR(255) NOT NULL, mot_de_passe VARCHAR(255) NOT NULL, role VARCHAR(255) NOT NULL, statut VARCHAR(50) NOT NULL, date_inscription DATETIME NOT NULL, reset_token VARCHAR(255) DEFAULT NULL, reset_token_expires_at DATETIME DEFAULT NULL, last_activity_at DATETIME DEFAULT NULL, pays VARCHAR(2) DEFAULT NULL, telephone VARCHAR(20) DEFAULT NULL, annees_experience INT DEFAULT NULL, face_descriptor JSON DEFAULT NULL, discr VARCHAR(255) NOT NULL, age INT DEFAULT NULL, sexe VARCHAR(10) DEFAULT NULL, etablissement VARCHAR(255) DEFAULT NULL, niveau VARCHAR(50) DEFAULT NULL, specialite VARCHAR(255) DEFAULT NULL, niveau_enseignement VARCHAR(255) DEFAULT NULL, UNIQUE INDEX UNIQ_8D93D649E7927C74 (email), PRIMARY KEY(id)) DEFAULT CHARACTER SET utf8mb4');
        $this->addSql('ALTER TABLE group_invitation ADD CONSTRAINT FK_26D00010FE54D947 FOREIGN KEY (group_id) REFERENCES study_group (id) ON DELETE CASCADE');
        $this->addSql('ALTER TABLE group_invitation ADD CONSTRAINT FK_26D00010A7B4A7E3 FOREIGN KEY (invited_by_id) REFERENCES user (id) ON DELETE CASCADE');
        $this->addSql('ALTER TABLE group_member ADD CONSTRAINT FK_A36222A8FE54D947 FOREIGN KEY (group_id) REFERENCES study_group (id) ON DELETE CASCADE');
        ...
        $this->addSql('ALTER TABLE group_post ADD CONSTRAINT FK_73D037FDFE54D947 FOREIGN KEY (group_id) REFERENCES study_group (id) ON DELETE CASCADE');
        $this->addSql('ALTER TABLE group_post ADD CONSTRAINT FK_73D037FDF675F31B FOREIGN KEY (author_id) REFERENCES user (id) ON DELETE CASCADE');
        ...
        $this->addSql('ALTER TABLE reactivation_request ADD CONSTRAINT FK_1B8F23C8A76ED395 FOREIGN KEY (user_id) REFERENCES user (id) ON DELETE CASCADE');
        $this->addSql('ALTER TABLE study_group ADD CONSTRAINT FK_32BA1425B03A8386 FOREIGN KEY (created_by_id) REFERENCES user (id) ON DELETE CASCADE');
    }
