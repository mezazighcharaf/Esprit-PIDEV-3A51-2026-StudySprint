...
class GroupMemberRepository extends ServiceEntityRepository
{
     ...
     * Returns array of ['membership' => GroupMember, 'memberCount' => int]
     */
    public function findGroupsByUserWithCounts(User $user): array
    {
        ...
        // Get group IDs
        $groupIds = array_map(fn(GroupMember $m) => $m->getGroup()->getId(), $memberships);

        // Batch query for member counts
        $memberCounts = $this->getEntityManager()
            ->createQuery('
                SELECT g.id as groupId, COUNT(m.id) as memberCount
        ...
        foreach ($memberCounts as $row) {
            $countsMap[$row['groupId']] = (int) $row['memberCount'];
        }
     ...
     * Returns array indexed by group ID
     */
