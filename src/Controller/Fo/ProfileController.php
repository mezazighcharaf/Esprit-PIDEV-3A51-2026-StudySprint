...
class ProfileController extends AbstractController
{
    ...
    {
        file_put_contents('profile_debug.log', "EDIT REACHED. User: " . ($this->getUser() ? $this->getUser()->getUserIdentifier() : 'NULL') . "\n", FILE_APPEND);
        /** @var \App\Entity\User $user */
        $user = $this->getUser();
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            if (!empty($dto->newPassword)) {
                if (empty($dto->currentPassword)) {
                    $this->addFlash('error', 'Le mot de passe actuel est requis pour changer de mot de passe.');
