...
class ChatbotController extends AbstractController
{
    ...
    public function askQuestion(int $groupId, int $postId, Request $request): JsonResponse
    {
        ...
        if (!$post || ($post->getGroup() && $post->getGroup()->getId() !== $group->getId())) {
        ...
        try {
            ...
            return $this->json([
                ...
                'comment' => [
                    ...
                    'createdAt' => ($createdAt = $botComment->getCreatedAt()) ? $createdAt->format('c') : date('c'),
                    'timeAgo' => ($createdAt = $botComment->getCreatedAt()) ? $this->formattingService->formatTimeAgo($createdAt) : 'à l\'instant',
        ...
        try {
            ...
            $recent = array_map(function (BotInteraction $i) {
                ...
                return [
                    ...
                    'response' => mb_substr($response, 0, 150) . (mb_strlen($response) > 150 ? '...' : ''),
                    'feedback' => $i->getFeedback(),
                    'tokensUsed' => $i->getTokensUsed(),
                    'responseTimeMs' => $i->getResponseTimeMs(),
                    'createdAt' => ($i->getCreatedAt() ?? new \DateTimeImmutable())->format('c'),
                ];
            }, $recentInteractions);
            ...
            if ($stats) {
                $statsData = [
                    ...
                    'avgResponseTime' => (int) round($stats->avgResponseTime),
